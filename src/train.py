...
def estimate_loss(model: GPT, dataset: ByteDataset, args) -> dict:
    model.eval()
    out = {}
    with torch.no_grad():
        for split in ['train','val']:
            ...
            for _ in range(args.eval_iters):
                xb, yb = dataset.get_batch(split,args.batch_size, args.device)
                _, loss = model(xb, yb)
...
def main():
    ...
    dataset = ByteDataset(args.data, block_size= args.block_size)
    model = GPT(tokenizer.vocab_size, args.block_size, args.n_layer, args.n_head, args.n_embd, args.dropout).to(args.device)

    if args.compile and hasattr(torch,'compile'):
        model = torch.compile(model)
    
    optimizer = torch.optim.AdamW(model.parameters(),args.lr, betas=(0.9,0.95), weight_decay=args.weight_decay)
    ...
    for step in range(1, args.steps + 1):
        xb, xy = dataset.get_batch('train', args.batch_size, args.device)
        ...
        if args.sample_every > 0 and step % args.sample_every == 0:
            start = torch.randint(low=0, high=len(dataset.train) - args.block_size - 1, size=(1,)).item()
            seed = dataset.train[start:start + args.block_size].unsqueeze(0).to(args.device)
            out = model.generate(seed, max_new_tokens=args.sample_tokens, temperature=args.temperature, top_k=args.top_k, top_p=args.top_p)
            txt = tokenizer.decode(out[0].cpu())
